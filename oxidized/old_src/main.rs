...
impl Atlas {
    fn new(rl: &RaylibHandle, thread: &RaylibThread) -> Atlas {
        Atlas {
            atlas: Image::gen_image_color(1024, 1024, Color::new(0, 0, 0, u8::MAX)),
            last_index: 0,
        }
    }
    fn _put_into_atlas(_sprite: Sprite) -> u32 {
        0
    }
}

struct Game {
    _raylib: RaylibHandle,
    _thread: RaylibThread,
}

impl Game {
    fn new(_raylib: RaylibHandle, _thread: RaylibThread) -> Game {
        Game { _raylib, _thread }
    }
}
